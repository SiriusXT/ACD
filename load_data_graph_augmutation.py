...
def split_data(data: pd.DataFrame) -> (pd.DataFrame, pd.DataFrame, pd.DataFrame):
    ...
    test_size = int(0.1 * data_size)  #16962
    bili = test_size/2/sum([sum(list(x.values())) for x in list(user_less_min_raing.values())])
    print(f"bilibili:{bili}")
    print(f"user_less_most_raing:{sum([sum(list(x.values())) for x in list(user_less_most_raing.values())])}")
    print(f"user_less_min_raing:{sum([sum(list(x.values())) for x in list(user_less_min_raing.values())])}")


    test_from_user_less_min_raing_size = int(sum([sum(list(x.values())) for x in list(user_less_min_raing.values())]) * bili)
    test_from_user_less_most_raing_size = int(test_size - test_from_user_less_min_raing_size)
    count_from_min = 0

    count_from_most = 0
    for index, row in tqdm(data.iterrows(), desc="Processing split"):
        rn = user_less_min_raing[row['item_id']]
        rn_most = user_less_most_raing[row['item_id']]
        if rn != {} and row["rating"] in rn.keys():
            if count_from_min <= test_from_user_less_min_raing_size:
                count_from_min += 1
        ...
        elif row["rating"] in rn_most.keys():
            if count_from_most <= test_from_user_less_most_raing_size:
                count_from_most += 1
                test_data = pd.concat([test_data, row.to_frame().T], ignore_index=True)
            else:
                train_data = pd.concat([train_data, row.to_frame().T], ignore_index=True)
    ...
    print(f"test_data.shape[0]:{test_data.shape[0]},{test_data.shape[0] / data_size}")
    print(f"valid_data.shape[0]:{valid_data.shape[0]},{valid_data.shape[0] / data_size}")
    print(f"train_data.shape[0]:{train_data.shape[0]},{train_data.shape[0] / data_size}")
